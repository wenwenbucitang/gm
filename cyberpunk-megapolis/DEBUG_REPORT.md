...
| 2 | **HTML 启动兜底** | `index.html` 不再用 `<script type="module" src>`，改为动态 `import('./main.js?v=6')`。catch 中：`console.error` 原始错误、更新 `loadMsg`、显示项目现有 `#err` 错误层（标题"启动失败"，正文含"请检查部署文件" + 浏览器返回的原始错误内容）、绑定"重试"按钮 `location.reload()`。`#err` z-index 40 高于加载层 30，不会被遮挡。 |
| 3 | **缓存版本统一 v6** | `index.html` 动态 import、`main.js` 内全部本地模块（player/controller/cameraRig/cityBoxes）、`cameraRig.js → cityBoxes.js` 全部 `?v=6`；`BUILD = '2026-07-23v6'`；运行时代码 grep 确认无 v7 残留、无混合引用。 |
...
| 用例 | 结果 |
|---|---|
| 正常加载进入角色选择界面 | ✅ 菜单可达，5 个 v6 模块全部 200，0 个 HTTP≥400 |
| **Network 中 input.js 请求数** | ✅ **0** |
| **`textures/CP_Plaster_N.webp`** | ✅ HTTP 200，119,854 字节 |
| 页面不存在永久加载 | ✅ 加载层按真实 LoadingManager 进度淡出 |
| 模拟 `main.js` 404 | ✅ `#err` 显示"启动失败 / 请检查部署文件 / Failed to fetch dynamically imported module: …/main.js?v=6"（含浏览器原始错误），重试按钮已绑定 |
...
2. 本次交付统一使用版本参数 `?v=6` 与 BUILD `2026-07-23v6`，部署时应整体替换目录，避免命中旧 CDN 缓存。
