...
## 二、修改内容（main.js / index.html / cameraRig.js，玩法与视觉零改动）
...
| 2 | **HTML 启动兜底** | `index.html` 不再用 `<script type="module" src>`，改为动态 `import('./main.js?v=8')`。catch 中：`console.error` 原始错误、更新 `loadMsg`、显示项目现有 `#err` 错误层（标题"启动失败"，正文含"请检查部署文件" + 浏览器返回的原始错误内容）、绑定"重试"按钮 `location.reload()`。`#err` z-index 40 高于加载层 30，不会被遮挡。 |
| 3 | **缓存版本统一 v8** | 战斗版新增 `combat.js` 后，`index.html` 动态 import、`main.js` 内全部本地模块（player/controller/cameraRig/cityBoxes/combat）、`cameraRig.js → cityBoxes.js` 全部 `?v=8`；`BUILD = '2026-07-23v8-combat'`；运行时代码 grep 确认无混合引用。 |
| 4 | **资源完整性全量审计** | 按代码引用 + `materials.json` + `chars/data/materials.json` + `textures_webp.json` 清单 + vendor/draco/fonts 全量核对：6 个运行时 JS 模块、15 个 vendor 文件（含 addons 依赖闭包）、3 个 Draco 解码文件、9+2+10 个 GLB、9 个 JSON、245+56 张纹理、字体、环境图——**0 缺失、0 空文件、0 内容损坏**（WebP RIFF、GLB 魔数、WASM/WOFF2 魔数逐一校验）。未删除任何资源。 |
...
| 6 | **能量刀战斗系统** | 男女角色右手均增加 E-EDGE 能量刀；`F` 或锁定后的鼠标左键挥砍。刀刃使用 HDR 发光、点光源和动态世界空间拖尾；`combat.js` 通过刀刃线段与目标碰撞体计算命中，每击 40 伤害。落地后部署 3 台 100 HP 训练无人机，包含血条、受击闪烁、火花、击毁和 4 秒重生。 |
...
| 用例 | 结果 |
|---|---|
| 正常加载进入角色选择界面 | ✅ 菜单可达，6 个 v8 模块全部 200，0 个 HTTP≥400 |
| **Network 中 input.js 请求数** | ✅ **0** |
| **`textures/CP_Plaster_N.webp`** | ✅ HTTP 200，119,854 字节 |
| 页面不存在永久加载 | ✅ 加载层按真实 LoadingManager 进度淡出 |
| 模拟 `main.js` 404 | ✅ `#err` 显示"启动失败 / 请检查部署文件 / Failed to fetch dynamically imported module: …/main.js?v=8"（含浏览器原始错误），重试按钮已绑定 |
...
2. 本次战斗版统一使用版本参数 `?v=8` 与 BUILD `2026-07-23v8-combat`，部署时应整体替换目录，避免命中旧 CDN 缓存。
